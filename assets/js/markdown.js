window.SQOLMarkdown = (() => {
  ...
  const inline = value => {
    let x = esc(value);
    const links = [];
    x = x.replace(/\[([^\]]+)\]\((https?:\/\/[^)]+)\)/g, (_, label, url) => {
      const token = `@@SQOLLINK${links.length}@@`;
      links.push(`<a href="${url}" target="_blank" rel="noopener">${label}</a>`);
      return token;
    });
    x = x.replace(/`([^`]+)`/g, '<code>$1</code>');
    x = x.replace(/\*\*([^*]+)\*\*/g, '<strong>$1</strong>');
    x = x.replace(/https?:\/\/[^\s<)]+/g, url => `<a href="${url}" target="_blank" rel="noopener">${url}</a>`);
    links.forEach((link, i) => { x = x.replace(`@@SQOLLINK${i}@@`, link); });
    return x;
  };
  function render(md) {
  ...
  function sections(md) {
    const lines = String(md || '').replace(/\r/g, '').split('\n');
    const sections = []; let current = null; const intro = [];
  ...
  return { render, sections, versionSections: md => sections(md).sections, inline };
})();
