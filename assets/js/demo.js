(() => {
  ...
  function defaultAppState() {
    return { route:'home', activeBotId:DEFAULT_BOT, hiddenBots:[], chatFolder:'All', persona:'Demo user', homeSearch:'', homeSort:'Trending', quickFilter:'', filtersOpen:false, creationSearch:'', personaSearch:'', botFlags:{}, editorAdvancedOpen:false, loreTab:'entries', activeLorebook:'wayfarer', activePersonaName:'Demo user', customPersonas:[], personaOverrides:{}, customLorebooks:[], lorebookOverrides:{} };
  }
  ...
  function renderPersonaEditor(isCreate){
    ...
    routeHost.innerHTML=`<section class="demo-view demo-editor-view" data-testid="PersonaEdit-PageStaticMetaTags"><div class="demo-view-toolbar"><div><h3>${isCreate?'Create Persona':'Edit '+escapeHTML(name)}</h3><p>This is a working local dummy editor based on the supplied Persona editor. Saving changes only the playground.</p></div><button data-demo-route="personas">Back to My Personas</button></div><div class="demo-editor-real demo-persona-editor"><div class="demo-avatar-upload"><div class="demo-persona-editor-avatar">${escapeHTML((name||'P')[0])}</div><div><strong>Persona image</strong><button data-demo-simulate="Persona image upload simulated locally.">Upload image</button></div></div><label>Persona name<input data-demo-persona-field="name" value="${escapeAttr(name)}"></label><label>Short note<input data-demo-persona-field="note" value="${escapeAttr(note)}"></label><label>Folder<input data-demo-persona-field="folder" value="${escapeAttr(folder)}"></label><label>Persona description<textarea data-demo-persona-field="description" rows="8">${escapeHTML(description)}</textarea></label><div class="demo-editor-save-row"><button data-demo-route="personas">Cancel</button><button data-demo-save-persona="${isCreate?'create':'edit'}">${isCreate?'Create Persona':'Save Persona'}</button></div></div></section>`;
  }
  ...
  function loreEntriesHTML(l){const entries=Array.from({length:Math.max(1,Math.min(8,Number(l.entries||0)||1))},(_,i)=>({name:['Setting overview','Important location','Character relationship','Rules of the world','History note','Keyword example','Faction note','Travel note'][i]||`Entry ${i+1}`,keys:['world, setting','shrine, temple','friend, rival','rule, custom','history, past','keyword, trigger','faction, group','road, travel'][i]||'demo'}));return `<div class="demo-lore-entry-toolbar"><button data-demo-simulate="A new local entry editor would open here; no server request is made.">＋ New Entry</button><button data-demo-simulate="Wiki/Web importer opened.">Wiki / Web Importer</button><button data-demo-simulate="Bulk selection opened.">Select entries</button></div><div class="demo-lore-entry-list">${entries.map((e,i)=>`<article><div><strong>${escapeHTML(e.name)}</strong><small>Keywords: ${escapeHTML(e.keys)}</small><p>Safe dummy Lorebook entry text used for list, full-text and bulk-tool demonstrations.</p></div><div><button data-demo-simulate="Entry duplicated locally.">Duplicate</button><button data-demo-simulate="Entry editor opened locally for ${escapeAttr(e.name)}.">Edit</button></div></article>`).join('')}</div>`;}
